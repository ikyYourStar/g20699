...
---@class MaxMpAttribute : CompositeAttribute
local MaxMpAttribute = class("MaxMpAttribute", CompositeAttribute)

--- 计算当前属性值
---@param baseValue number 基础属性
function MaxMpAttribute:calculateFinalValue(baseValue)
    local value = CompositeAttribute.calculateFinalValue(self, baseValue)
    --- 向上取整
    return GameLib.keepPreciseDecimal(value, 1)
end

return MaxMpAttribute